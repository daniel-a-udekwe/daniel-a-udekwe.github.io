...
beIn the following, we will learn about three common discrete probability distributions: The Bernoulli, binomial and geometric distribution.
...
Since the event of success *k* canoccur anywhere in *n* trials, we have [*\"n choose k\"*](https://en.wikipedia.org/wiki/Binomial_coefficient) ways to distribute the success.
...
This is mainly due to the [central limit theorem](https://en.wikipedia.org/wiki/Central_limit_theorem), which, loosely speaking, states that the average of many independent random variables with finite mean and variance is itself a random variable --- which is normally distributed as the number of observations increases.
